...
namespace SRTS
{
    ...
    nameof(DrawBombingTargeter)));
            ...
            harmony.Patch(AccessTools.Method(typeof(TransferableUtility), nameof(TransferableUtility.Transfer)), new HarmonyMethod(typeof(StartUp), nameof(BeforeTransfer)));
            harmony.Patch(AccessTools.Method("RimWorld.Dialog_FormCaravan+<>c__DisplayClass94_0:<AddItemsFromTransferablesToRandomInventories>b__3"), new HarmonyMethod(typeof(StartUp), nameof(TransferDelegate)));
        ...
        public static IEnumerable<Gizmo> LaunchAndBombGizmosPassthrough(IEnumerable<Gizmo> __result, Caravan __instance)
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                if ((element as Command_Action)?.defaultLabel == "CommandSettle".Translate() && CaravanInventoryUtility.AllInventoryItems(__instance).Select(x => x is MinifiedThing minified ? minified.InnerThing : x).Any(y => y.TryGetComp<CompLaunchableSRTS>() != null))
                {
                    float massUsage = 0f;
                    Thing srts = null;
                    foreach (Pawn p in __instance.PawnsListForReading)
                    {
                        ...
                        {
                            if (t is MinifiedThing minifiedThing && (minifiedThing.InnerThing?.HasComp<CompLaunchableSRTS>() ?? false))
                                srts = minifiedThing.InnerThing;
                            else if (t.TryGetComp<CompLaunchableSRTS>() != null)
                                srts = t;
                            else
                            {
        ...
        private static void BeforeTransfer(List<Thing> things)
        {
            thingsFromSRTS = new Dictionary<Thing, IEnumerable<Thing>>();
            foreach (var thing in things)
            {
                var transporter = thing.TryGetComp<CompTransporter>();
                if (thing.HasComp<CompLaunchableSRTS>() && transporter != null)
                {
                    thingsFromSRTS.Add(thing, transporter.GetDirectlyHeldThings().ToArray().Select(x => x.SplitOff(x.stackCount)).ToList());
                }
            }
        }
        static Dictionary<Thing, IEnumerable<Thing>> thingsFromSRTS = null;
        public static bool TransferDelegate(Thing splitPiece, IThingHolder originalHolder, List<Pawn> ___pawns)
        {
            if (splitPiece.HasComp<CompLaunchableSRTS>())
            {
                CaravanInventoryUtility.FindPawnToMoveInventoryTo(splitPiece, ___pawns, null, null).inventory.TryAddAndUnforbid(splitPiece);
                var thingHolder = splitPiece.TryGetComp<CompTransporter>().GetDirectlyHeldThings();
                if (thingsFromSRTS.TryGetValue(splitPiece, out IEnumerable<Thing> containedThings))
                {
                    thingHolder.TryAddRangeOrTransfer(containedThings);
                }
                return false;
            }
            return true;
