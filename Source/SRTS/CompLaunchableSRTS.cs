...
namespace SRTS
{
    ...
    public class CompLaunchableSRTS : ThingComp
    {
        ...
        public void WorldStartChoosingDestination(Caravan car)
        {
            ...
            int tile = car.Tile;
            this.carr = car;
            Find.WorldTargeter.BeginTargeting(new Func<GlobalTargetInfo, bool>(this.ChoseWorldTarget), true, CompLaunchableSRTS.TargeterMouseAttachment, false, (Action)(() => GenDraw.DrawWorldRadiusRing(car.Tile, this.MaxLaunchDistance)), (target =>
            {
                if (!target.IsValid)
                    return (string)null;
